...
#' List all temporary tables in SQL Server that are associated with the
#' database connection.
#'
#' @param db_connection An SQL server database connection.
#' @param global_temp_tables T/F whether to include global temporary tables
#' (ones with names that start '##').
#' 
#' @returns Character array containing the names of all temporary tables
#' associated with the current connection.
#' 
#' @details
#' Uses the SQL query:
#' `SELECT name FROM tempdb..sysobjects WHERE OBJECT_ID('tempdb..' + name) IS NOT NULL`
#' 
#' @export
fetch_all_sql_server_temp_tables = function(db_connection, global_temp_tables = FALSE){
  stopifnot(DBI::dbIsValid(db_connection))
  stopifnot(any(grepl("sql ?server", class(db_connection), ignore.case = TRUE)))
  stopifnot(global_temp_tables %in% c(TRUE, FALSE))
  
  query = glue::glue(
    "SELECT name\n",
    "FROM tempdb..sysobjects\n", # all temporary tables
    "WHERE OBJECT_ID('tempdb..' + name) IS NOT NULL\n", # from connection
    ifelse(global_temp_tables, "AND name NOT LIKE '##%'", "")
  )
  
  tmp_tables = DBI::dbGetQuery(db_connection, query)
  return(tmp_tables$name)
}

## Drop SQL Server tables ------------------------------------------------- ----
#' Drop tables from SQL Server
#' 
#' @param db_connection An SQL server database connection.
#' @param tables A character array containing a list of tables to drop.
#' @param if_exists T/F whether to include 'IF EXISTS' in the SQL query.
#' Defaults to TRUE, if FALSE the command will error if any entry in `tables`
#' does not exist in the database.
#' 
#' @returns An invisible numeric array of the same length as `tables` with 1 if
#' the table was dropped, and 0 otherwise.
#' 
#' @export
drop_sql_server_tables = function(db_connection, tables, if_exists = TRUE){
  stopifnot(DBI::dbIsValid(db_connection))
  stopifnot(any(grepl("sql ?server", class(db_connection), ignore.case = TRUE)))
  ...
  results = sapply(
    tables,
    function(tbl){
      query = glue::glue("DROP TABLE {if_exists} {tbl}")
      result = DBI::dbExecute(db_connection, query)
      return(result)
    }
  )
